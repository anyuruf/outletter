...
const CarouselButtonGroup = (props: Props) => {
	...
	return (
		<>
			...
			<div
				className={cn(
					"absolute inline-flex w-fit divide-x divide-primary-foreground/30 rounded-md shadow-xs",
					className
				)}
			>
				<Tooltip>
				...
				<Tooltip>
					<TooltipTrigger asChild>
						<Button
							type="button"
							onClick={() => (isPlaying ? autoplay.current?.stop() : autoplay.current?.play())}
							className="rounded-none rounded-r-full focus-visible:z-10"
						>
							{isPlaying ? (
								<PauseIcon className="text-shadow" strokeWidth={3} />
							) : (
								<PlayIcon className="text-shadow" strokeWidth={3} />
							)}
							<span className="sr-only">{isPlaying ? "Pause" : "Play"}</span>
						</Button>
					</TooltipTrigger>

					<TooltipContent className="px-2 py-1 text-xs">{isPlaying ? "Pause" : "Play Slides"}</TooltipContent>
				</Tooltip>
			</div>
		</>
	)
